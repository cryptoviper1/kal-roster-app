  ...
  try {
    ...
    if (events.length > 0) {
      ...
      const eventsToDelete = existingEvents.filter(ev => {
        const isOurs = ev.extendedProperties?.private?.source === 'kalroster' || 
                       (ev.description && (
                         ev.description.includes('KAL Roster') || 
                         ev.description.includes('Parsed by KAL Roster App') || 
                         ev.description.includes('Rotation Per Diem') || 
                         ev.description.includes('Flight Details:')
                       ));
        return isOurs;
      });
    ...
    for (const ev of events) {
      // Clean arbitrary UI fields
      delete ev.type; 
      
      // Inject identifying property for future deletions
